...
def verifica_se_ha_duplicacoes():
    duplicacoes = list()

    for lista in lista_de_listas_de_inteiros:
        if len(lista) != len(set(lista)):
            duplicacoes.append(verifica_duplicados(lista))
        else:
            duplicacoes.append(-1)
...
def verifica_duplicados(lista):
    ...
    for elemento in lista:
        if not ocorrencias.get(elemento):
            ocorrencias[elemento] = 0
        for numero in ocorrencias.keys():
            if elemento == numero:
                ocorrencias[numero] += 1
        if ocorrencias[elemento] == 2:
            return elemento
    
print(verifica_se_ha_duplicacoes())
